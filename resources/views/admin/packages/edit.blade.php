...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h2 class="text-base font-bold text-gray-800">Edit Informasi Paket</h2>
        ...
        <form method="POST" action="{{ route('admin.packages.update', $package->id) }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $package->name) }}"
                       ...
                       class="w-full px-3.5 py-2.5 rounded-xl border text-sm transition-colors outline-none
                              {{ $errors->has('name') ? 'border-red-400 bg-red-50 focus:border-red-500 focus:ring-2 focus:ring-red-200' : 'border-gray-200 bg-gray-50 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-100' }}"
                       required>
                @error('name')
            ...
            {{-- Nama Tampilan --}}
            <div>
                <label for="display_name" class="block text-sm font-semibold text-gray-700 mb-1.5">
                ...
                <input type="text" name="display_name" id="display_name"
                       ...
                       placeholder="contoh: Paket Basic"
                       class="w-full px-3.5 py-2.5 rounded-xl border text-sm transition-colors outline-none
                              {{ $errors->has('display_name') ? 'border-red-400 bg-red-50 focus:border-red-500 focus:ring-2 focus:ring-red-200' : 'border-gray-200 bg-gray-50 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-100' }}"
                       required>
                @error('display_name')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          ...
                          class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm transition-colors outline-none focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-100 resize-none">{{ old('description', $package->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="price" id="price"
                           ...
                           class="w-full px-3.5 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm transition-colors outline-none focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-100"
                           required>
                    @error('price')
                        <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex items-center gap-3 p-3.5 rounded-xl bg-gray-50 border border-gray-200">
                ...
                <label for="is_free" class="text-sm font-semibold text-gray-700 cursor-pointer select-none">
                    Paket Gratis
            ...
            <div>
                ...
                <input type="file" name="thumbnail" id="thumbnail" accept=".jpg,.jpeg,.png"
                       class="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 border border-gray-200 rounded-xl px-2 py-1.5">
                @if($package->thumbnail)
            ...
            <div class="pt-2">
                <button type="submit"
                        class="inline-flex items-center gap-2 bg-green-700 hover:bg-green-800 text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-colors">
