...
@section('content')
    ...
    @if(session('success'))
        <div
            class="mb-6 flex items-start gap-3 bg-yellow-50 border border-yellow-200 text-yellow-900 rounded-xl px-4 py-3.5 text-sm font-medium">
            <svg class="w-5 h-5 text-yellow-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-yellow-50 bg-[#faf9f6]">
                        <th class="text-left text-xs font-bold text-gray-500 uppercase tracking-wider px-6 py-3.5">Nama
                            Paket</th>
                        <th class="text-left text-xs font-bold text-gray-500 uppercase tracking-wider px-4 py-3.5">Kategori
                        </th>
                        <th class="text-left text-xs font-bold text-gray-500 uppercase tracking-wider px-4 py-3.5">Harga
                        </th>
                        <th class="text-left text-xs font-bold text-gray-500 uppercase tracking-wider px-4 py-3.5">Status
                        </th>
                        <th class="text-center text-xs font-bold text-gray-500 uppercase tracking-wider px-4 py-3.5">Materi
                        </th>
                        <th class="text-right text-xs font-bold text-gray-500 uppercase tracking-wider px-6 py-3.5">Aksi
                        </th>
                    </tr>
                </thead>
                    ...
                    @forelse($packages ?? [] as $package)
                        <tr class="hover:bg-yellow-50/20 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    @if($package->thumbnail)
                                        <img src="{{ Storage::url($package->thumbnail) }}" alt="{{ $package->display_name }}"
                                            class="w-11 h-11 rounded-xl object-cover flex-shrink-0 border border-yellow-100/60 shadow-sm">
                                    @else
                                        <div
                                            class="w-11 h-11 rounded-xl bg-yellow-50 border border-yellow-100 flex items-center justify-center flex-shrink-0">
                            ...
                            <td class="px-4 py-4">
                                @if($package->kategori)
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg bg-yellow-100 text-yellow-900 text-xs font-semibold">
                                        {{ $package->kategori }}
                                    </span>
                                @else
                            ...
                            <td class="px-4 py-4">
                                @if($package->is_free)
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[11px] font-extrabold">
                                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Gratis
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-100 text-amber-800 text-[11px] font-extrabold">
                            ...
                            <td class="px-4 py-4 text-center">
                                <span
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-gray-100 text-gray-700 text-xs font-extrabold">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    {{-- <a href="{{ route('admin.packages.show', $package->id) }}"
                                        class="inline-flex items-center gap-1 text-xs font-bold text-yellow-800 hover:text-yellow-950 bg-yellow-100 hover:bg-yellow-200 px-3 py-1.5 rounded-lg transition-colors">
                                        Detail
                                    </a> --}}
